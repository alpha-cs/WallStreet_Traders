...
{
	std::string sql_table[] = { maketable_company_info, maketable_td_price_history_day, maketable_td_price_history_minute };
	for (int i = 0; i < sizeof_sql_tdameritrade_table; i++)
	{
		if ((sql_table_status & (1 << i)) == 0)
		{
			try
			{
				...
				std::cout << "Table " << i << " created." << std::endl;
			}
			catch (const std::exception& e)
			{
				std::cerr << e.what() << '\n';
...
dbStatus load_sql_database::check_company_info()
{
	return check_table("company_info");
}
